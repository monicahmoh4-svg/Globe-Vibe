...
import { MessageCircle, Search, Globe2 } from 'lucide-react'
  ...
  return (
    <div className="p-5 max-w-2xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-sub">{connections.length} conversation{connections.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-600 text-sm mb-5">Browse foreigners and start chatting</p>
          <Link to="/browse" className="btn-primary inline-flex text-sm py-2 px-5">Browse Foreigners</Link>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map(conn => {
              ...
              <Link key={conn.id} to={active ? `/chat/${conn.id}` : '#'}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {expired && <span className="badge badge-orange text-xs">Expired</span>}
                  </div>
