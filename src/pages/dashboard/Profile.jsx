...
import { User, Globe2, MessageCircle, Star, Edit3, Save, X, Lock, ChevronDown, DollarSign, CheckCircle, AlertCircle } from 'lucide-react'
...
export default function Profile() {
  ...
  const [fgLoading, setFgLoad]= useState(false)
  ...
  const changePwd = async e => {
    ...
    if (pwdForm.newPassword !== pwdForm.confirmPassword) {
      setPwdMsg({ type: 'error', text: "Passwords don't match" }); return
    }
    setPwdSave(true)
    try {
      ...
      setPwdMsg({ type: 'error', text: e.response?.data?.error || 'Failed' })
    } finally { setPwdSave(false) }
  }

  const handleBecomeForeigner = async () => {
    ...
    try {
      ...
      await refreshUser()
      setTimeout(() => setShowForeigner(false), 2500)
    } catch(e) {
      setFgMsg({ type: 'error', text: e.response?.data?.error || 'Failed' })
  ...
  return (
    <div className="p-5 max-w-xl mx-auto">
      ...
      {msg.text && (
        <div className={`rounded-xl p-3 mb-4 text-sm flex items-center gap-2 ${msg.type === 'success'
      ...
      <div className="card border border-white/5 mb-4">
        <div className="flex items-start gap-4">
          <div className="relative flex-shrink-0">
            ...
            {user.is_online && (
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-400 rounded-full border-2 border-dark-800 animate-pulse"/>
            )}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 text-xs text-gray-500 flex-wrap">
              <span className="flex items-center gap-1">{flag(user.country_code)} {user.country}</span>
              <span className="flex items-center gap-1"><MessageCircle size={11}/> {user.total_chats} chats</span>
              {user.is_foreigner && (
                <span className="flex items-center gap-1"><Star size={11} className="text-yellow-400 fill-yellow-400"/> {Number(user.rating).toFixed(1)}</span>
              )}
      ...
      <div className={`card border mb-4 ${user.is_foreigner ? 'border-purple-500/20 bg-purple-500/5' : 'border-white/5'}`}>
        <button onClick={() => setShowForeigner(v => !v)}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left">
              <div className="font-display font-semibold text-white text-sm">
                {user.is_foreigner ? 'Foreigner Status — Active' : 'Become a Foreigner'}
              </div>
              <div className="text-xs text-gray-500">
                {user.is_foreigner
                  ? 'You appear in the browse list. Users pay to connect with you.'
                  : 'Get listed so users can pay to chat with you'}
              </div>
            </div>
          </div>
          <ChevronDown size={16} className={`text-gray-500 transition-transform ${showForeigner ? 'rotate-180' : ''}`}/>
        </button>
              ...
              <>
                <div className="flex items-center gap-2 p-3 bg-purple-500/10 rounded-xl border border-purple-500/20">
                  ...
                  <p className="text-xs text-purple-300">
                    You are currently listed as a foreigner. Users can browse your profile and pay to start chatting with you.
                  </p>
                </div>
                {fgMsg.text && (
                  <div className={`text-sm p-3 rounded-xl ${fgMsg.type === 'success'
                    ? 'bg-green-500/10 text-green-400 border border-green-500/20'
                    : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                    {fgMsg.text}
                  </div>
                )}
                <button onClick={handleBecomeForeigner} disabled={fgLoading}
                  className="btn-secondary text-sm py-2.5 w-full text-red-400 hover:text-red-300 hover:bg-red-500/10">
                  {fgLoading
                    ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>
                    : 'Remove me from Foreigners list'}
                </button>
              </>
            ) : (
              <>
                <p className="text-xs text-gray-400 leading-relaxed">
                  By registering as a foreigner, your profile will appear in the browse page. Users will pay an activation fee to chat with you. This helps users connect with people from around the world.
                </p>
                <div>
                  <label className="text-xs text-gray-400 mb-1.5 block">Update your bio</label>
                  <textarea value={fgForm.bio} onChange={e => setFgForm(f => ({ ...f, bio: e.target.value }))}
                    rows={2} placeholder="Tell users about yourself and your culture…"
                    className="input-field text-sm resize-none"/>
                </div>
                <div>
                  <label className="text-xs text-gray-400 mb-1.5 block">Languages you speak</label>
                  <input type="text" value={fgForm.languages}
                    onChange={e => setFgForm(f => ({ ...f, languages: e.target.value }))}
                    className="input-field text-sm" placeholder="e.g. French, English, Spanish"/>
                </div>
                <div>
                  <label className="text-xs text-gray-400 mb-1.5 block">Why do you want to join? (optional)</label>
                  <input type="text" value={fgForm.motivation}
                    onChange={e => setFgForm(f => ({ ...f, motivation: e.target.value }))}
                    className="input-field text-sm" placeholder="e.g. I want to meet people from Kenya and share my culture"/>
                </div>
                {fgMsg.text && (
                  <div className={`text-sm p-3 rounded-xl ${fgMsg.type === 'success'
                    ? 'bg-green-500/10 text-green-400 border border-green-500/20'
                ...
                <button onClick={handleBecomeForeigner} disabled={fgLoading}
                  ...
                  {fgLoading
                    ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>
                    : <><Globe2 size={14}/> Register as Foreigner</>}
                </button>
              </>
            )}
          </div>
            ...
            {[
              ...
              <div key={field}>
                ...
                <input type="password" value={pwdForm[field]}
                  ...
                  className="input-field text-sm" placeholder="••••••" required
                  minLength={field === 'currentPassword' ? 1 : 6}/>
              </div>
            ))}
            {pwdMsg.text && (
              <div className={`text-sm p-3 rounded-xl ${pwdMsg.type === 'success'
                ? 'bg-green-500/10 text-green-400 border border-green-500/20'
                : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                {pwdMsg.text}
              </div>
